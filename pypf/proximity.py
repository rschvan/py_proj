...
class Proximity:
    ...
    def _calculate_stats(self):
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    distance = self.dismat[i, j]
                    if not np.isinf(distance) and not np.isnan(distance):
                        off_diagonal_distances.append(distance)
        if off_diagonal_distances:
            self.mean = np.mean(off_diagonal_distances)
            self.sd = np.std(off_diagonal_distances)
            self.min = np.min(off_diagonal_distances)
            self.max = np.max(off_diagonal_distances)
...
if __name__ == "__main__":
    from pypf.utility import prxprint
    #   Example with filepath (you'll need to create test files)
    #   proximity_obj_from_path_sq = Proximity("square_with_header.xlsx")
    #   if proximity_obj_from_path_sq.terms:
    #       prxprint(proximity_obj_from_path_sq.dismat, "Square With Header")

    #   proximity_obj_from_path_rect = Proximity("rectangular_no_header.csv")
    #   if proximity_obj_from_path_rect.terms:
    #       prxprint(proximity_obj_from_path_rect.dismat, "Rectangular No Header")

    #   Example with direct data
    example_terms = ["A", "B", "C"]
    example_dismat = np.array([[np.nan, 2, np.inf], [2, np.nan, 4], [np.inf, 4, np.nan]])
    proximity_obj_direct = Proximity(terms=example_terms, dismat=example_dismat)
    if proximity_obj_direct.terms:
        prxprint(proximity_obj_direct, "Direct Data")