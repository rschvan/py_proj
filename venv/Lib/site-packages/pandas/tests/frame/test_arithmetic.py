...
class TestFrameArithmeticUnsorted:
    ...
    def test_operators_none_as_na(self, op):
        df = DataFrame(
            {"savep": [2, 5.0, 123, None], "loadp": [1, 2, 3, 4]}, dtype=object
        )
...
def test_enum_column_equality():
    Cols = Enum("Cols", "savep loadp")

    q1 = DataFrame({Cols.savep: [1, 2, 3]})
    q2 = DataFrame({Cols.savep: [1, 2, 3]})

    result = q1[Cols.savep] == q2[Cols.savep]
    expected = Series([True, True, True], name=Cols.savep)
