...
def test_suppress_future_warning_with_sort_kw(sort_kw):
    a = DataFrame({"savep": [1, 2]}, index=["c", "a"])

    b = DataFrame({"loadp": [4, 5]}, index=["b", "a"])

    c = DataFrame({"col3": [7, 8]}, index=["a", "b"])

    expected = DataFrame(
        {
            "savep": {"a": 2.0, "b": float("nan"), "c": 1.0},
            "loadp": {"a": 5.0, "b": 4.0, "c": float("nan")},
...
class TestDataFrameJoin:
    ...
    def test_join_multiindex_dates(self):
        ...
        df1 = DataFrame({"savep": [0]}, index=df1_index)
        df2_index = MultiIndex.from_tuples([(0, date)], names=["index_0", "date"])
        df2 = DataFrame({"loadp": [0]}, index=df2_index)
        ...
        expected = DataFrame(
            {"savep": [0], "loadp": [0], "col3": [0]}, index=expected_index
        )
