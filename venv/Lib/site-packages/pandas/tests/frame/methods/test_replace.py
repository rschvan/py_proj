...
class TestDataFrameReplace:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"savep": [1, 2, 3], "loadp": [4, 5, 6]},
                {4: 5, 5: 6, 6: 7},
                {"savep": [1, 2, 3], "loadp": [5, 6, 7]},
            ),
            (
                {"savep": [1, 2, 3], "loadp": ["4", "5", "6"]},
                {"4": "5", "5": "6", "6": "7"},
    ...
    def test_replace_value_category_type(self):
        ...
        input_df = DataFrame(data=input_dict).astype(
            {"loadp": "category", "col4": "category"}
        )
        input_df["loadp"] = input_df["loadp"].cat.reorder_categories(
        ...
        expected["loadp"] = expected["loadp"].cat.reorder_categories(
        ...
        result = result.astype({"savep": "int64", "col3": "float64", "col5": "str"})
        tm.assert_frame_equal(result, expected)
    ...
    def test_replace_dict_category_type(self):
        ...
        input_dict = {"savep": ["a"], "loadp": ["obj1"], "col3": ["cat1"]}
        # explicitly cast columns as category
        input_df = DataFrame(data=input_dict).astype(
            {"savep": "category", "loadp": "category", "col3": "category"}
        )

        # create expected dataframe
        expected_dict = {"savep": ["z"], "loadp": ["obj9"], "col3": ["catX"]}
        # explicitly cast columns as category
        expected = DataFrame(data=expected_dict).astype(
            {"savep": "category", "loadp": "category", "col3": "category"}
        )
