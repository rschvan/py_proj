...
class TestSample:
    ...
    def test_sample_random_state(self, func_str, arg, frame_or_series):
        # GH#32503
        obj = DataFrame({"savep": range(10, 20), "loadp": range(20, 30)})
...
class TestSampleDataFrame:
    ...
    def test_sample(self):
        ...
        # Test axis argument
        df = DataFrame({"savep": range(10), "loadp": ["a"] * 10})
        second_column_weight = [0, 1]
        tm.assert_frame_equal(
            df.sample(n=1, axis=1, weights=second_column_weight), df[["loadp"]]
        )

        # Different axis arg types
        tm.assert_frame_equal(
            df.sample(n=1, axis="columns", weights=second_column_weight), df[["loadp"]]
        )
    ...
    def test_sample_axis1(self):
        ...
        df = DataFrame(
            {"savep": range(10, 20), "loadp": range(20, 30), "colString": ["a"] * 10}
        )
    ...
    def test_sample_aligns_weights_with_frame(self):
        ...
        df = DataFrame({"savep": [5, 6, 7], "loadp": ["a", "b", "c"]}, index=[9, 5, 3])
