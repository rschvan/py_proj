...
class TestDataFrameRound:
    def test_round(self):
        ...
        df = DataFrame({"savep": [1.123, 2.123, 3.123], "loadp": [1.234, 2.234, 3.234]})

        # Default round to integer (i.e. decimals=0)
        expected_rounded = DataFrame({"savep": [1.0, 2.0, 3.0], "loadp": [1.0, 2.0, 3.0]})
        tm.assert_frame_equal(df.round(), expected_rounded)

        # Round with an integer
        decimals = 2
        expected_rounded = DataFrame(
            {"savep": [1.12, 2.12, 3.12], "loadp": [1.23, 2.23, 3.23]}
        )
        ...
        expected_rounded = DataFrame(
            {"savep": [1.1, 2.1, 3.1], "loadp": [1.23, 2.23, 3.23]}
        )
        round_dict = {"savep": 1, "loadp": 2}
        tm.assert_frame_equal(df.round(round_dict), expected_rounded)

        # Incomplete dict
        expected_partially_rounded = DataFrame(
            {"savep": [1.123, 2.123, 3.123], "loadp": [1.2, 2.2, 3.2]}
        )
        partial_round_dict = {"loadp": 1}
        tm.assert_frame_equal(df.round(partial_round_dict), expected_partially_rounded)

        # Dict with unknown elements
        wrong_round_dict = {"col3": 2, "loadp": 1}
        tm.assert_frame_equal(df.round(wrong_round_dict), expected_partially_rounded)

        # float input to `decimals`
        non_int_round_dict = {"savep": 1, "loadp": 0.5}
        msg = "Values in decimals must be integers"
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)

        # String input
        non_int_round_dict = {"savep": 1, "loadp": "foo"}
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)
        ...
        # List input
        non_int_round_dict = {"savep": 1, "loadp": [1, 2]}
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)
        ...
        # Negative numbers
        negative_round_dict = {"savep": -1, "loadp": -2}
        big_df = df * 100
        expected_neg_rounded = DataFrame(
            {"savep": [110.0, 210, 310], "loadp": [100.0, 200, 300]}
        )
        tm.assert_frame_equal(big_df.round(negative_round_dict), expected_neg_rounded)

        # nan in Series round
        nan_round_Series = Series({"savep": np.nan, "loadp": 1})

        with pytest.raises(TypeError, match=msg):
            df.round(nan_round_Series)

        # Make sure this doesn't break existing Series.round
        tm.assert_series_equal(df["savep"].round(1), expected_rounded["savep"])
        ...
        tm.assert_series_equal(df["savep"].round(decimals), expected_rounded["savep"])
        tm.assert_series_equal(df.round(decimals)["savep"], expected_rounded["savep"])
    ...
    def test_round_mixed_type(self):
        ...
        tm.assert_frame_equal(df.round({"savep": 1}), df)
        tm.assert_frame_equal(df.round({"savep": 0}), round_0)
        tm.assert_frame_equal(df.round({"savep": 0, "loadp": 1}), round_0)
