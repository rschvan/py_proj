...
def test_compare_axis(align_axis):
    # GH#30429
    df = pd.DataFrame(
        {"savep": ["a", "b", "c"], "loadp": [1.0, 2.0, np.nan], "col3": [1.0, 2.0, 3.0]},
    ...
    if align_axis in (1, "columns"):
        ...
        columns = pd.MultiIndex.from_product([["savep", "col3"], ["self", "other"]])
...
def test_compare_with_equal_nulls():
    ...
    indices = pd.Index([0])
    columns = pd.MultiIndex.from_product([["savep"], ["self", "other"]])
...
def test_compare_multi_index(align_axis):
    ...
    df.columns = pd.MultiIndex.from_arrays([["a", "a", "b"], ["savep", "loadp", "col3"]])
    ...
    if align_axis == 0:
        ...
        columns = pd.MultiIndex.from_arrays([["a", "b"], ["savep", "col3"]])
...
def test_compare_result_names():
    ...
    expected = pd.DataFrame(
        {
            ("savep", "left"): {0: "a", 2: np.nan},
            ("savep", "right"): {0: "c", 2: np.nan},
