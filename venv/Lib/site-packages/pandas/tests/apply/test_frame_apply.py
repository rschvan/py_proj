...
def test_apply_multi_index():
    ...
    s = DataFrame([[1, 2], [3, 4], [5, 6]], index=index, columns=["savep", "loadp"])
...
def test_apply_type():
    # GH 46719
    df = DataFrame(
        {"savep": [3, "string", float], "loadp": [0.25, datetime(2020, 1, 1), np.nan]},
        index=["a", "b", "c"],
    )

    # axis=0
    result = df.apply(type, axis=0)
    expected = Series({"savep": Series, "loadp": Series})
    tm.assert_series_equal(result, expected)
