...
def test_transform_length():
    # GH 9697
    df = DataFrame({"savep": [1, 1, 2, 2], "loadp": [1, 2, 3, np.nan]})
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        results = [
            df.groupby("savep").transform(sum)["loadp"],
            df.groupby("savep")["loadp"].transform(sum),
            df.groupby("savep").transform(nsum)["loadp"],
            df.groupby("savep")["loadp"].transform(nsum),
        ]
