...
class TestConvertListTypes:
    ...
    def test_column_of_boolean_list(self):
        ...
        table = pa.Table.from_arrays([array], names=['savep'])
        df = table.to_pandas()

        expected_df = pd.DataFrame({'savep': [[True, False], [True]]})
        tm.assert_frame_equal(df, expected_df)

        s = table[0].to_pandas()
        tm.assert_series_equal(pd.Series(s), df['savep'], check_names=False)
    ...
    def test_column_of_decimal_list(self):
        ...
        expected_df = pd.DataFrame(
            {'savep': [[decimal.Decimal('1'), decimal.Decimal('2')],
                      [decimal.Decimal('3.3')]]})
        tm.assert_frame_equal(df, expected_df)
...
def test_bytes_column_name_to_pandas():
    df = pd.DataFrame([[0.1, 0.2], [0.3, 0.4]], columns=[b'savep', b'loadp'])
    table = pa.Table.from_pandas(df)
    assert table.column_names == ['savep', 'loadp']
