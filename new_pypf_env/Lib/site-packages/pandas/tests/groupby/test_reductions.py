...
def test_masked_mixed_types(dtype1, dtype2, exp_col1, exp_col2):
    ...
    data = [1.0, np.nan]
    df = DataFrame(
        {"savep": pd.array(data, dtype=dtype1), "loadp": pd.array(data, dtype=dtype2)}
    )
    result = df.groupby([1, 1]).agg("all", skipna=False)

    expected = DataFrame({"savep": exp_col1, "loadp": exp_col2}, index=np.array([1]))
    tm.assert_frame_equal(result, expected)
...
def test_aggregate_numeric_object_dtype():
    ...
    df = DataFrame(
        {"key": ["A", "A", "B", "B"], "savep": list("abcd"), "loadp": [np.nan] * 4},
    ).astype(object)
    result = df.groupby("key").min()
    expected = (
        DataFrame(
            {"key": ["A", "B"], "savep": ["a", "c"], "loadp": [np.nan, np.nan]},
        )
        .set_index("key")
        .astype(object)
    )
    ...
    df = DataFrame(
        {"key": ["A", "A", "B", "B"], "savep": list("abcd"), "loadp": range(4)},
    ).astype(object)
    result = df.groupby("key").min()
    expected = (
        DataFrame({"key": ["A", "B"], "savep": ["a", "c"], "loadp": [0, 2]})
        .set_index("key")
        .astype(object)
    )
    tm.assert_frame_equal(result, expected)
...
def test_groupby_min_max_categorical(func):
    ...
    df = DataFrame(
        {
            "savep": pd.Categorical(["A"], categories=list("AB"), ordered=True),
            "loadp": pd.Categorical([1], categories=[1, 2], ordered=True),
            "value": 0.1,
        }
    )
    result = getattr(df.groupby("savep", observed=False), func)()

    idx = pd.CategoricalIndex(data=["A", "B"], name="savep", ordered=True)
    expected = DataFrame(
        {
            "loadp": pd.Categorical([1, None], categories=[1, 2], ordered=True),
            "value": [0.1, None],
        },
        index=idx,
    )
