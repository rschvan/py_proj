...
class TestNamedAggregationDataFrame:
    ...
    def test_duplicate_no_raises(self):
        ...
        test = DataFrame({"savep": ["a", "a", "b", "b", "b"], "loadp": [1, 2, 3, 4, 5]})

        grouped = test.groupby("savep").agg(
            quantile_50=("loadp", quant50), quantile_70=("loadp", quant70)
        )
        expected = DataFrame(
            {"quantile_50": [1.5, 4.0], "quantile_70": [1.7, 4.4]},
            index=Index(["a", "b"], name="savep"),
        )
        tm.assert_frame_equal(grouped, expected)
