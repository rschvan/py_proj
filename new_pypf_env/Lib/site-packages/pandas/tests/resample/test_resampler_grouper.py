...
def test_apply_with_mutated_index():
    ...
    df = DataFrame(
        data={"savep": np.random.default_rng(2).random(len(index))}, index=index
    )
    ...
    # A case for series
    expected = df["savep"].groupby(pd.Grouper(freq="ME"), group_keys=False).apply(f)
    result = df["savep"].resample("ME").apply(f)
    tm.assert_series_equal(result, expected)
...
def test_resample_groupby_with_label(unit):
    ...
    df = DataFrame(index=index, data={"col0": [0, 0, 1, 1, 2], "savep": [1, 1, 1, 1, 1]})
    msg = "DataFrameGroupBy.resample operated on the grouping columns"
    with tm.assert_produces_warning(FutureWarning, match=msg):
    ...
    expected = DataFrame(
        data={"col0": [0, 0, 2, 2], "savep": [1, 1, 2, 1]}, index=mindex
    )
...
def test_apply_to_one_column_of_df():
    # GH: 36951
    df = DataFrame(
        {"col": range(10), "savep": range(10, 20)},
...
def test_resample_groupby_agg_object_dtype_all_nan(consolidate):
    ...
    df1 = DataFrame({"key": "A", "date": dates, "savep": range(15), "col_object": "val"})
    df2 = DataFrame({"key": "B", "date": dates, "savep": range(15)})
