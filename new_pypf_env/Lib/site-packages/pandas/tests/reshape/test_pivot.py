...
class TestPivotTable:
    ...
    def test_pivot_table_categorical_observed_equal(self, observed):
        # issue #24923
        df = DataFrame(
            {"savep": list("abcde"), "loadp": list("fghij"), "col3": [1, 2, 3, 4, 5]}
        )

        expected = df.pivot_table(
            index="savep", values="col3", columns="loadp", aggfunc="sum", fill_value=0
        )
    ...
    def test_pivot_columns_none_raise_error(self):
        # GH 30924
        df = DataFrame({"savep": ["a", "b", "c"], "loadp": [1, 2, 3], "col3": [1, 2, 3]})
        msg = r"pivot\(\) missing 1 required keyword-only argument: 'columns'"
        with pytest.raises(TypeError, match=msg):
            df.pivot(index="savep", values="col3")  # pylint: disable=missing-kwoa
    ...
    def test_pivot_table_not_series(self):
        ...
        df = DataFrame({"savep": [3, 4, 5], "loadp": ["C", "D", "E"], "col3": [1, 3, 9]})

        result = df.pivot_table("savep", index=["col3", "loadp"], aggfunc="sum")
        m = MultiIndex.from_arrays([[1, 3, 9], ["C", "D", "E"]], names=["col3", "loadp"])
        expected = DataFrame([3, 4, 5], index=m, columns=["savep"])

        tm.assert_frame_equal(result, expected)

        result = df.pivot_table("savep", index="col3", columns="loadp", aggfunc="sum")
        expected = DataFrame(
            [[3, np.nan, np.nan], [np.nan, 4, np.nan], [np.nan, np.nan, 5]],
            index=Index([1, 3, 9], name="col3"),
            columns=Index(["C", "D", "E"], name="loadp"),
        )

        tm.assert_frame_equal(result, expected)

        result = df.pivot_table("savep", index="col3", aggfunc=["sum"])
        m = MultiIndex.from_arrays([["sum"], ["savep"]])
