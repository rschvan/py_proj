...
def test_error_with_prefix_multiple_seperators():
    ...
    with pytest.raises(
        ValueError,
        match=(r"Separator not specified for column: loadp-a"),
    ):
...
def test_error_with_prefix_default_category_dict_not_complete(
    ...
    with pytest.raises(
        ...
        from_dummies(dummies_with_unassigned, sep="_", default_category={"savep": "x"})
...
def test_roundtrip_with_prefixes():
    categories = DataFrame({"savep": ["a", "b", "a"], "loadp": ["b", "a", "c"]})
    dummies = get_dummies(categories)
    result = from_dummies(dummies, sep="_")
    expected = categories
    tm.assert_frame_equal(result, expected)
...
def test_with_prefix_basic(dummies_basic):
    expected = DataFrame({"savep": ["a", "b", "a"], "loadp": ["b", "a", "c"]})
...
def test_with_prefix_contains_get_dummies_NaN_column():
    ...
    expected = DataFrame({"savep": ["a", "b", "NaN"], "loadp": ["NaN", "a", "c"]})
    result = from_dummies(dummies, sep="_")
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "x",
            DataFrame({"savep": ["a", "b", "x"], "loadp": ["x", "a", "c"]}),
            id="default_category is a str",
        ),
        pytest.param(
            0,
            DataFrame({"savep": ["a", "b", 0], "loadp": [0, "a", "c"]}),
            id="default_category is a 0",
        ),
        pytest.param(
            False,
            DataFrame({"savep": ["a", "b", False], "loadp": [False, "a", "c"]}),
            id="default_category is a False",
        ),
        pytest.param(
            {"loadp": 1, "savep": 2.5},
            DataFrame({"savep": ["a", "b", 2.5], "loadp": [1, "a", "c"]}),
            id="default_category is a dict with int and float values",
        ),
        pytest.param(
            {"loadp": None, "savep": False},
            DataFrame({"savep": ["a", "b", False], "loadp": [None, "a", "c"]}),
            id="default_category is a dict with bool and None values",
        ),
        pytest.param(
            {"loadp": (1, 2), "savep": [1.25, False]},
            DataFrame({"savep": ["a", "b", [1.25, False]], "loadp": [(1, 2), "a", "c"]}),
...
def test_ea_categories_with_sep():
    ...
    expected = DataFrame(
        {
            "savep": Series(list("aba"), dtype="string[python]"),
            "loadp": Series(list("bac"), dtype="string[python]"),
        }
    )
