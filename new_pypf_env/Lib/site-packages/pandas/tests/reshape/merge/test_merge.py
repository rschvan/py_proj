...
def dfs_for_indicator():
    df1 = DataFrame({"savep": [0, 1], "col_conflict": [1, 2], "col_left": ["a", "b"]})
    df2 = DataFrame(
        {
            "savep": [1, 2, 3, 4, 5],
...
class TestMerge:
    ...
    def test_indicator(self, dfs_for_indicator):
        ...
        test = merge(df1, df2, on="savep", how="outer", indicator=True)
        tm.assert_frame_equal(test, df_result)
        test = df1.merge(df2, on="savep", how="outer", indicator=True)
        tm.assert_frame_equal(test, df_result)
        ...
        test_custom_name = merge(
            df1, df2, on="savep", how="outer", indicator="custom_name"
        )
        tm.assert_frame_equal(test_custom_name, df_result_custom_name)
        test_custom_name = df1.merge(
            df2, on="savep", how="outer", indicator="custom_name"
        )
        tm.assert_frame_equal(test_custom_name, df_result_custom_name)
    ...
    def test_merge_indicator_arg_validation(self, dfs_for_indicator):
        ...
        with pytest.raises(ValueError, match=msg):
            merge(df1, df2, on="savep", how="outer", indicator=5)
        with pytest.raises(ValueError, match=msg):
            df1.merge(df2, on="savep", how="outer", indicator=5)

    def test_merge_indicator_result_integrity(self, dfs_for_indicator):
        # Check result integrity
        df1, df2 = dfs_for_indicator

        test2 = merge(df1, df2, on="savep", how="left", indicator=True)
        assert (test2._merge != "right_only").all()
        test2 = df1.merge(df2, on="savep", how="left", indicator=True)
        assert (test2._merge != "right_only").all()

        test3 = merge(df1, df2, on="savep", how="right", indicator=True)
        assert (test3._merge != "left_only").all()
        test3 = df1.merge(df2, on="savep", how="right", indicator=True)
        assert (test3._merge != "left_only").all()

        test4 = merge(df1, df2, on="savep", how="inner", indicator=True)
        assert (test4._merge == "both").all()
        test4 = df1.merge(df2, on="savep", how="inner", indicator=True)
        assert (test4._merge == "both").all()
    ...
    def test_merge_indicator_invalid(self, dfs_for_indicator):
        ...
        for i in ["_right_indicator", "_left_indicator", "_merge"]:
            df_badcolumn = DataFrame({"savep": [1, 2], i: [2, 2]})
            ...
            with pytest.raises(ValueError, match=msg):
                merge(df1, df_badcolumn, on="savep", how="outer", indicator=True)
            with pytest.raises(ValueError, match=msg):
                df1.merge(df_badcolumn, on="savep", how="outer", indicator=True)

        # Check for name conflict with custom name
        df_badcolumn = DataFrame({"savep": [1, 2], "custom_column_name": [2, 2]})
        ...
        with pytest.raises(ValueError, match=msg):
            df1.merge(
                df_badcolumn, on="savep", how="outer", indicator="custom_column_name"
            )

    def test_merge_indicator_multiple_columns(self):
        # Merge on multiple columns
        df3 = DataFrame({"savep": [0, 1], "loadp": ["a", "b"]})

        df4 = DataFrame({"savep": [1, 1, 3], "loadp": ["b", "x", "y"]})

        hand_coded_result = DataFrame(
            {"savep": [0, 1, 1, 3], "loadp": ["a", "b", "x", "y"]}
        )
        ...
        test5 = merge(df3, df4, on=["savep", "loadp"], how="outer", indicator=True)
        tm.assert_frame_equal(test5, hand_coded_result)
        test5 = df3.merge(df4, on=["savep", "loadp"], how="outer", indicator=True)
        tm.assert_frame_equal(test5, hand_coded_result)
...
@pytest.mark.parametrize(
    "savep, loadp, kwargs, expected_cols",
    [
...
def test_merge_outer_with_NaN(dtype):
    # GH#43550
    left = DataFrame({"key": [1, 2], "savep": [1, 2]}, dtype=dtype)
    right = DataFrame({"key": [np.nan, np.nan], "loadp": [3, 4]}, dtype=dtype)
    result = merge(left, right, on="key", how="outer")
    expected = DataFrame(
        {
            "key": [1, 2, np.nan, np.nan],
            "savep": [1, 2, np.nan, np.nan],
            "loadp": [np.nan, np.nan, 3, 4],
        },
        dtype=dtype,
    )
    tm.assert_frame_equal(result, expected)
