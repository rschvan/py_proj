...
class TestDataFrameConstructors:
    ...
    def test_constructor_dtype_copy(self):
        orig_df = DataFrame({"savep": [1.0], "loadp": [2.0], "col3": [3.0]})

        new_df = DataFrame(orig_df, dtype=float, copy=True)

        new_df["savep"] = 200.0
        assert orig_df["savep"][0] == 1.0
    ...
    def test_constructor_dict(self):
        ...
        frame = DataFrame({"savep": datetime_series, "loadp": datetime_series_short})

        # loadp is padded with NaN
        assert len(datetime_series) == 30
        assert len(datetime_series_short) == 25

        tm.assert_series_equal(frame["savep"], datetime_series.rename("savep"))
        ...
        tm.assert_series_equal(exp, frame["loadp"])

        frame = DataFrame(
            {"savep": datetime_series, "loadp": datetime_series_short},
            columns=["loadp", "col3", "col4"],
        )

        assert len(frame) == len(datetime_series_short)
        assert "savep" not in frame
    ...
    def test_constructor_subclass_dict(self, dict_subclass):
        ...
        data = {
            "savep": dict_subclass((x, 10.0 * x) for x in range(10)),
            "loadp": dict_subclass((x, 20.0 * x) for x in range(10)),
        }
        df = DataFrame(data)
    ...
    def test_constructor_ordered_dict_nested_preserve_order(self):
        ...
        data = OrderedDict([("loadp", nested1), ("savep", nested2)])
        result = DataFrame(data)
        data = {"loadp": [1, 2], "savep": [2, 5]}
