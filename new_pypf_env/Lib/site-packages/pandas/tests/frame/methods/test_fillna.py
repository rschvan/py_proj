...
class TestFillNA:
    ...
    def test_fillna_downcast_dict(self):
        # GH#40809
        df = DataFrame({"savep": [1, np.nan]})

        msg = "The 'downcast' keyword in fillna"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.fillna({"savep": 2}, downcast={"savep": "int64"})
        expected = DataFrame({"savep": [1, 2]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_single_block_df_with_horizontal_axis(self):
        ...
        df = DataFrame(
            {
                "savep": [5, 0, np.nan, 10, np.nan],
                "loadp": [7, np.nan, np.nan, 5, 3],
