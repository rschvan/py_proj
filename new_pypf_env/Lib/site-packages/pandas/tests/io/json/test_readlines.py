...
def test_to_json_append_orient(orient_):
    ...
    df = DataFrame({"savep": [1, 2], "loadp": ["a", "b"]})
    msg = (
...
def test_to_json_append_output_consistent_columns():
    ...
    df1 = DataFrame({"savep": [1, 2], "loadp": ["a", "b"]})
    df2 = DataFrame({"savep": [3, 4], "loadp": ["c", "d"]})

    expected = DataFrame({"savep": [1, 2, 3, 4], "loadp": ["a", "b", "c", "d"]})
    with tm.ensure_clean("test.json") as path:
...
def test_to_json_append_output_different_columns():
    ...
    expected = DataFrame(
        {
            "savep": [1, 2, 3, 4, None, None, None, None],
            "loadp": ["a", "b", "c", "d", "e", "f", np.nan, np.nan],
    ...
    expected = DataFrame(
        {
            ...
            "loadp": [np.nan, np.nan, "e", "f", "c", "d", "a", "b"],
            "col3": [np.nan, np.nan, "!", "#", np.nan, np.nan, np.nan, np.nan],
            "savep": [None, None, None, None, 3, 4, 1, 2],
        }
    ).astype({"col4": "float"})
    with tm.ensure_clean("test.json") as path:
        # Save dataframes to the same file
